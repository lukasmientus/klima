...
function makeBeakerModel(kind){
  return {
    ...
    rimLm: 112,
    rockTopLm: 100,
    kFast: 0.80,  // Landeis: schneller bis Steine bedeckt
    kSlow: 0.32,  // danach wie Meereis
    _inner: null,
    _waterY: null,
    overflowParticles: [],
  };
...
function newCubeAt(x, y){
  const c = {
    ...
    placed: false,
    container: null, // 'A'|'B'
    phase: 'free',   // 'air'|'water'|'stone'|'free'
...
function updateIce(dt){
  ...
  for (const c of cubes){
    ...
    if (c.container === 'A'){
      const k = (model.A.levelLm < model.A.rockTopLm) ? model.A.kFast : model.A.kSlow;
      model.A.levelLm += melted * k;
    } else if (c.container === 'B'){
      model.B.levelLm += melted * model.B.kSlow;
    }
  }
  ...
  for (const b of [model.A, model.B]){
    if (b.levelLm > b.rimLm){
      const overshoot = b.levelLm - b.rimLm;
      b.levelLm = b.rimLm;
      if (b._inner){
...
/** Cube motion:
 * - In machine (home==='machine' and not placed): stays where it is.
 * - If fallingOut: gravity until it leaves the canvas, then removed.
 * - If placed: in air it falls; once in water it rises to float; in A with exposed rocks it lands on stone.
 */
function stepCubeMotion(dt){
  ...
  for (let i=cubes.length-1; i>=0; i--){
    ...
    if (!c.placed && c.fallingOut){
      const gOut = 3200;
      c.vy += gOut * (dt/1000);
      // no damping outside the beakers -> clearly accelerated fall
      c.y += c.vy * (dt/1000);
  ...
  for (const c of cubes){
    ...
    const waterY = b._waterY ?? lmToInnerY(inner, b.levelLm);
    ...
    if (c.container === 'A' && rocksExposed){
      // always land on rock if it sticks out
      c.phase = 'air';
      c.vy += g * (dt/1000);
      c.vy *= damp;
      c.y += c.vy * (dt/1000);
    ...
    // in water: rise to float
    c.phase = 'water';
    const targetY = waterY - h/3;
...
/** Ruler with start/current pointers */
...
/** Rock stack A: top rock clearly above 80 lm, top near 100 lm; 6 rocks */
function drawRockStackA(beakerRect, b){
  const inner = b._inner;
  if (!inner) return;

  const floorY = inner.y + inner.h;
  const water80Y = lmToInnerY(inner, 80);
  ...
  const xFracs = [0.50, 0.60, 0.42, 0.58, 0.46, 0.54];
  const rots   = [0.02, -0.20, 0.18, -0.16, 0.12, -0.10];
  ...
  // ensure water80 cuts through upper region
  const upper = rocks[N-1];
  const upperBottom = upper.y + upper.h*footF;
  if (water80Y < upperBottom - H*0.15){
    const adjust = (upperBottom - H*0.15) - water80Y;
    for (const r of rocks){ r.y -= adjust; }
  }
...
/** Placement (important behavior):
 * - Drop in machine -> stays.
 * - Drop over beaker opening (x within inner) and above inner -> falls into beaker.
 * - Drop inside beaker: keep y (so deep drops rise to surface).
 * - Drop elsewhere: falls out of scene.
 */
function placeCube(c){
  ...
  function dropInto(which, inner){
    ...
    if (above){
      // start above opening for visible falling into the beaker
      c.y = inner.y - hit.h;
    } else if (inside){
      // keep y: if dropped deeper, buoyancy will raise it
      // but clamp to beaker interior
      c.y = clamp(c.y, inner.y - hit.h*0.4, inner.y + inner.h - hit.h*0.6);
    }

    if (which === 'A' && model.A.startLm === null) model.A.startLm = model.A.levelLm;
    if (which === 'B' && model.B.startLm === null) model.B.startLm = model.B.levelLm;
  }
...
function drawBeaker(beakerRect, b, label){
  ...
  const fillTxt = `Füllmenge: ${b.levelLm.toFixed(1)} lm`;
  const dTxt = (b.startLm !== null) ? `Δh: ${(b.levelLm - b.startLm).toFixed(1)} lm` : `Δh: –`;
  ctx.save();
  ctx.fillStyle = '#1f1f1f';
  ctx.font = '16px Arial';
  ctx.textAlign = 'center';
